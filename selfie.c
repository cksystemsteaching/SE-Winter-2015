...
//// code of Sched_yield according to linux system
...
/// Implimentation of  syscalls sched_yield 
void syscall_yield() {
    int* process;

    process = pollListHead(readyProcesses);
    saveProcessState(process);
    removeFirst(readyProcesses);
    appendListElement(process, readyProcesses);
	...
	if(*head != 0){
 	...
 	while (1) {
		if(counterInstructions == instructionsPerSwitch){
			...
			putchar(10);
			print((int*) " current process switch to next process!!! :");
			putchar(10);
