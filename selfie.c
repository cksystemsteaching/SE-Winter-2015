...
void createSymbolTableEntry(int which, int *string, int line, int class,
		int type, int value, int reference);
...
int* contextInsert(int contextId, int pc, int* registers, int* page_table,
		int* prev, int* next);
int* contextFindContextByPID(int* contexts, int contextId);
...
int getSymbol() {
	...
	if (isCharacterLetter()) {
		...
		if (character == CHAR_EOF) {
			syntaxErrorMessage(
					(int*) "reached end of file looking for a character constant");
...
void createSymbolTableEntry(int whichTable, int *string, int line, int class,
		int type, int value, int reference) {
	int *newEntry;
...
int load_variable(int *variable) {
	...
	emitIFormat(OP_LW, getRegister(entry), currentTemporary(),
			getReference(entry));
...
void load_integer() {
	...
	if (constant >= 0) {
		if (constant < twoToThePowerOf(15))
			...
			emitIFormat(OP_ADDIU, REG_ZR, currentTemporary(),
					rightShift(constant, 14));

			// shift left by 14 bits
			emitLeftShiftBy(14);

			// and finally add 14 lsbs
			emitIFormat(OP_ADDIU, currentTemporary(), currentTemporary(),
					rightShift(leftShift(constant, 18), 18));
		} else {
			// load 14 msbs of a 31-bit number first
			emitIFormat(OP_ADDIU, REG_ZR, currentTemporary(),
					rightShift(constant, 17));

			emitLeftShiftBy(14);

			// then add the next 14 msbs
			emitIFormat(OP_ADDIU, currentTemporary(), currentTemporary(),
					rightShift(leftShift(constant, 15), 18));

			emitLeftShiftBy(3);

			// and finally add the remaining 3 lsbs
			emitIFormat(OP_ADDIU, currentTemporary(), currentTemporary(),
					rightShift(leftShift(constant, 29), 29));
		}
	} else {
...
void load_string() {
	...
	createSymbolTableEntry(GLOBAL_TABLE, string, lineNumber, STRING, INTSTAR_T,
			0, -allocatedMemory);
...
int help_call_codegen(int *entry, int *procedure) {
	...
	if (entry == (int*) 0) {
		...
		createSymbolTableEntry(GLOBAL_TABLE, procedure, lineNumber, FUNCTION,
				INT_T, 0, binaryLength);
...
int gr_term() {
	...
	while (isStarOrDivOrModulo()) {
		...
		if (operatorSymbol == SYM_ASTERISK) {
			emitRFormat(OP_SPECIAL, previousTemporary(), currentTemporary(), 0,
					FCT_MULTU);
...
int gr_simpleExpression() {
	...
	if (sign == 1) {
		...
		emitRFormat(OP_SPECIAL, REG_ZR, currentTemporary(), currentTemporary(),
				FCT_SUBU);
	}
	...
	while (isPlusOrMinus()) {
		...
		if (operatorSymbol == SYM_PLUS) {
			...
			emitRFormat(OP_SPECIAL, previousTemporary(), currentTemporary(),
					previousTemporary(), FCT_ADDU);
...
int gr_expression() {
	...
	if (isComparison()) {
		...
		if (operatorSymbol == SYM_EQUALITY) {
			...
			emitRFormat(OP_SPECIAL, previousTemporary(), currentTemporary(),
					previousTemporary(), FCT_SLT);

			tfree(1);

		} else if (operatorSymbol == SYM_GT) {
			// set to 1 if b < a, else 0
			emitRFormat(OP_SPECIAL, currentTemporary(), previousTemporary(),
					previousTemporary(), FCT_SLT);
...
void gr_statement() {
		...
		if (symbol == SYM_IDENTIFIER) {
			...
			if (symbol == SYM_RPARENTHESIS) {
				...
				if (symbol == SYM_ASSIGN) {
					...
					emitIFormat(OP_SW, previousTemporary(), currentTemporary(),
							0);
	...
	// identifier "=" expression | call
		...
		if (symbol == SYM_LPARENTHESIS) {
			...
			emitIFormat(OP_SW, getRegister(entry), currentTemporary(),
					getReference(entry));
	...
	// return statement?
		...
		entry = getSymbolTableEntry(currentProcedureName, FUNCTION,
				global_symbol_table);
...
void gr_variable(int offset) {
	...
	if (symbol == SYM_IDENTIFIER) {
		createSymbolTableEntry(LOCAL_TABLE, identifier, lineNumber, VARIABLE,
				type, 0, offset);
		...
		createSymbolTableEntry(LOCAL_TABLE, (int*) "missing variable name",
				lineNumber, VARIABLE, type, 0, offset);
	}
}
...
void gr_initialization(int *name, int offset, int type) {
	...
	createSymbolTableEntry(GLOBAL_TABLE, name, actualLineNumber, VARIABLE, type,
			initialValue, offset);
}
...
void gr_procedure(int *procedure, int returnType) {
	...
	if (symbol == SYM_SEMICOLON) {
		...
		if (entry == (int*) 0)
			createSymbolTableEntry(GLOBAL_TABLE, currentProcedureName,
					lineNumber, FUNCTION, returnType, 0, 0);
		...
		if (entry == (int*) 0)
			createSymbolTableEntry(GLOBAL_TABLE, currentProcedureName,
					lineNumber, FUNCTION, returnType, 0, binaryLength);
		else {
			if (getReference(entry) != 0) {
				...
				if (symbol == SYM_LPARENTHESIS)
					...
					if (symbol == SYM_SEMICOLON) {
						createSymbolTableEntry(GLOBAL_TABLE,
								variableOrProcedureName, lineNumber, VARIABLE,
								type, 0, -allocatedMemory);

						getSymbol();

						// type identifier "=" global variable definition
					} else
						gr_initialization(variableOrProcedureName,
...
void emitMainEntry() {
	...
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "main", 0, FUNCTION, INT_T, 0,
			binaryLength);
...
void emitWrite() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "write", 0, FUNCTION, INT_T, 0,
			binaryLength);
...
void emitOpen() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "open", 0, FUNCTION, INT_T, 0,
			binaryLength);
...
void emitMalloc() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "malloc", 0, FUNCTION,
			INTSTAR_T, 0, binaryLength);
...
void emitPMem() {
	...
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "pmem", 0, FUNCTION, INTSTAR_T,
			0, binaryLength);
...
void emitPutchar() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "putchar", 0, FUNCTION, INT_T,
			0, binaryLength);
...
void copyRegisters (int* oldRegisters, int* newRegisters) {
	int* cursor;
...
void emitLock() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "lock", 0, FUNCTION, INTSTAR_T,
			0, binaryLength);
...
void emitYield() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "yield", 0, FUNCTION, INTSTAR_T,
			0, binaryLength);
...
int* remove_context(int* context_list, int pid) {
	int* next;
	int* prev;
	int* node;
	...
	next = (int*) context_getNext(node);
	prev = (int*) context_getPrev(node);

	if ((int) prev != 0) {
		context_setNext(prev, (int) next);
	} else {
		context_list = next;
	}

	if ((int) next != 0) {
		context_setPrev(next, (int) prev);
	}
}
...
void emitCreateContext() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "create_context", 0, FUNCTION,
			INTSTAR_T, 0, binaryLength);
...
void emitSwitchContext() {
	...
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "switch_context", 0, FUNCTION,
			INTSTAR_T, 0, binaryLength);
...
void emitMapPageInContext() {
	...
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "map_page_in_context", 0,
			FUNCTION, INTSTAR_T, 0, binaryLength);
...
void copyPageFrame(int* parentFrame, int* childFrame) {
	int* cursor;
