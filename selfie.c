...
void os_removeNodeFromReadyQ(int* node);
...
void syscall_lock()
{
    ...
    if (os_lock == 1) {
        if (debug_lock) {
            ...
            print((int*)", [Blocked] ");
            print(itoa(os_blockedCtr + 1, string_buffer, 10, 0, 0));
            print((int*)", [ready] ");
            print(itoa(os_readyCtr - 1, string_buffer, 10, 0, 0));
            println();
        }
        os_lockProcess = 1;
    }
...
void syscall_unlock()
{
    ...
    if ((int)os_lockQ != 0) {
        waitForChild = os_getListEntry(7, os_lockQ);
        listStart = os_lockQ;
        while (waitForChild != 0) {
            os_lockQ = (int*)os_getNextNode(os_lockQ);
            waitForChild = os_getListEntry(7, os_lockQ);
            if ((int)os_lockQ == (int)listStart) {
                return;
            }
        }
...
void syscall_wait()
{
    //First check if childs are running
    int childCntr;
    int pId;
    pId = os_getListEntry(4, os_readyQ);
    ...
    if (childCntr > 0) {
        if (debug_fork) {
            println();
            print((int*)"Parent with [PID] ");
            print(itoa(pId, string_buffer, 10, 0, 0));
            print((int*)" wait for it's childs now");
            println();
        }
        os_setListEntry(7, 1, os_readyQ);
        os_lockProcess = 1;
    }
    else {
        if (debug_fork) {
            ...
            print((int*)"Parent with [PID] ");
            print(itoa(pId, string_buffer, 10, 0, 0));
            print((int*)" has no child to wait for");
            println();
        }
    }
}
...
void syscall_exit()
{
    ...
    *(registers + REG_V0) = exitCode;
    ...
    if ((int)parentNode != 0) {
        ...
        childCntr = os_getListEntry(6, parentNode);
        childCntr = childCntr - 1;
        os_setListEntry(6, childCntr, parentNode);
        //Check if Parent is Waiting and if parent has childs left
        pIsWaiting = os_getListEntry(7, parentNode);
        parentPId = os_getListEntry(4, parentNode);
        if (childCntr == 0) {
            if (pIsWaiting == 1) {
                println();
                print((int*)"Parent with [PID]");
                print(itoa(parentPId, string_buffer, 10, 0, 0));
                print((int*)"has no child left, move to readyQ now!");
                println();
                os_setListEntry(7, 0, parentNode);
                os_move2readyQ(parentNode);
            }
        }
    }
    if ((int)os_readyQ == 0) {
...
void os_createProcess()
{
    ...
    if ((int)os_readyQ == 0) {
        ...
        newP = os_addNodeToLList(7, os_readyQ);
        os_pId = os_pId + 1;
    }
...
int os_createFork()
{
    ...
    os_setListEntry(5, (int)os_readyQ, newPr);
...
{
    int* pTable;
    pTable = os_getListEntry(3, os_readyQ);
