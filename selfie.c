...
void create_process(int argc, int* argv) {
	...
	*(registers + REG_SP) = 4194300;
	...
	if (proc_count == 0) {
		proc_list = insert_process(pc, registers, current_page_table, 0, 0,
				proc_count);
...
void load_string() {
	...
	createSymbolTableEntry(GLOBAL_TABLE, string, -allocatedMemory, STRING,
			INTSTAR_T, 0);
...
int help_call_codegen(int *entry, int *procedure) {
	...
	if (entry == (int*) 0) {
		...
		createSymbolTableEntry(GLOBAL_TABLE, procedure, binaryLength, FUNCTION,
				INT_T, 0);
...
void gr_initialization(int *name, int offset, int type) {
	...
	createSymbolTableEntry(GLOBAL_TABLE, name, offset, VARIABLE, type,
			initialValue);
}
...
void gr_procedure(int *procedure, int returnType) {
	...
	if (symbol == SYM_SEMICOLON) {
		...
		if (entry == (int*) 0)
			createSymbolTableEntry(GLOBAL_TABLE, currentProcedureName, 0,
				...
				if (symbol == SYM_LPARENTHESIS)
					...
					if (symbol == SYM_SEMICOLON) {
						...
						createSymbolTableEntry(GLOBAL_TABLE,
								variableOrProcedureName, -allocatedMemory,
								VARIABLE, type, 0);
...
void emitMainEntry() {
	...
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "main", binaryLength, FUNCTION,
			INT_T, 0);
...
void compile() {
	...
	sourceFD = open(sourceName, O_RDONLY, 0); // 0 = O_RDONLY
...
void load() {
	...
	fd = open(binaryName, O_RDONLY, 0); // 0 = O_RDONLY
...
void emitExit() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "exit", binaryLength, FUNCTION,
			INT_T, 0);
...
void emitRead() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "read", binaryLength, FUNCTION,
			INT_T, 0);
...
void emitWrite() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "write", binaryLength, FUNCTION,
			INT_T, 0);
...
void emitOpen() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "open", binaryLength, FUNCTION,
			INT_T, 0);
...
void emitMalloc() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "malloc", binaryLength,
			FUNCTION, INTSTAR_T, 0);
...
void emitPutchar() {
	createSymbolTableEntry(GLOBAL_TABLE, (int*) "putchar", binaryLength,
			FUNCTION, INT_T, 0);
...
void restore_context() {
	...
	current_page_table = (int*) *(current_page_table + 2);
	pc = *current_proc;
